...
$(document).ready(function() {
    $('#content h2 .add').click(function() {
		popload('添加广告',900,500,"<?php echo U('Home/Ad/addAd');?>");
		addDiv($('#iframe_pop'));
		popclose();
	});
	$('#content #table .tr .edit').click(function(event) {
		...
		if (id=='' || isNaN(id)) {
			...
			popload('修改广告',740,420,"<?php echo U('Home/Ad/editAd','','');?>/id/"+id);
			addDiv($('#iframe_pop'));
			popclose();
		}
	});
	...
	$('#dely').click(function(event) {
		...
		if (delid=='') {
			...
			wintq('正在删除，请稍后...',4,20000,0,'');
			$.ajax({
				url:"<?php echo U('Home/Ad/del','','');?>/all/1",
				dataType:'JSON',
				type:'POST',
				data:{id:delid},
				success: function(data) {
...
<div id="content">
    ...
    <h2>
    	<div class="h2_left">
        	<a href="/route/index.php/Home/Ad/index" class="whole">全部</a>
    ...
    <table id="table" border="1" bordercolor="#CCCCCC" cellpadding="0" cellspacing="0">
        ...
        <?php if($ad_list == 0): ?><tr class="tr"><td class="tc" colspan="11">暂无数据，等待添加～！</td></tr><?php endif; ?>
        <!--顶级数据-->
        <?php if(is_array($ad_list)): $i = 0; $__LIST__ = $ad_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr class="tr <?php if(($mod) == "1"): ?>tr2<?php endif; ?>">
            ...
            <td class="tc"><?php echo (getStoreName($list["sid"])); ?></td>
            <td class="tc"><?php echo ($list["title"]); ?></td>
            <td class="tc"><?php echo ($list["link"]); ?></td>
            <td class="tc"><?php echo ($list["image"]); ?></td>
            <td class="tc"><?php echo ($list["create_user"]); ?></td>
